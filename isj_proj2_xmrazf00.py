...
def solfege(title_hymn):
    ...
    possible_title, sep, hymn = title_hymn.partition(":") if (title_hymn.find(':') + 1) else ['','',title_hymn]  # vase reseni

    # the hymn as a list of strings separated by ' '
    hymn_list = hymn.strip().split(' ')  # vase reseni

    # skipping always two strings, and ending 3 strings from the end
    skip2 = hymn_list[:len(hymn_list) - 2: 3]  # vase reseni
